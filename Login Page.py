...
def Signup():  # This is the signup definition,
    ...
    fname = Label(roots, text='First Name: ')  # This just does the same as above, instead with the text new username.
    lname = Label(roots, text='Last Name: ')  # ^^
    ...
    fname = Entry(roots)  # This now puts a text box waiting for input.
    lname = Entry(roots,
                  show='')  # Same as above, yet 'show=""' What this does is replace the text with *, like a password box :D
    fname.grid(row=1, column=1)  # You know what this does now :D
    lname.grid(row=2, column=1)

    email = Label(roots, text='Email ID: ')  # This just does the same as above, instead with the text new username.
    contact = Label(roots, text='Mobile Number: ')  # ^^
    email.grid(row=3, column=0,
               sticky=W)  # Same thing as the instruction var just on different rows. :) Tkinter is like that.
    contact.grid(row=4, column=0, sticky=W)  # ^^

    email = Entry(roots)  # This now puts a text box waiting for input.
    contact = Entry(roots,show='')  # Same as above, yet 'show=""' What this does is replace the text with *, like a password box :D
    email.grid(row=3, column=1)  # You know what this does now :D
    ...
    pwordE = Entry(roots,
                   show='')  # Same as above, yet 'show=""' What this does is replace the text with *, like a password box :D
    nameE.grid(row=5, column=1)  # You know what this does now :D
    pwordE.grid(row=6, column=1)
...
def CheckLogin():
    ...
    if nameEL.get() == uname and pwordEL.get() == pword:  # Checks to see if you entered the correct data.
        ...
        rlbl = messagebox.showinfo('Message','Logged In Successfully!')  # "logged in" label
        rlbl.pack()  # Pack is like .grid(), just different

    else:

        rlbl =  messagebox.showinfo('Warning','Invalid Login')
        rlbl.pack()
