...
def insertOrUpdate(Id, Name, roll) :                                            # this function is for database
    ...
    isRecordExist = 0
    for row in c:                                                          # checking wheather the id exist or not
        isRecordExist = 1
    if isRecordExist == 1:                                                      # updating name and roll no
        c.execute("UPDATE Students SET Name = ? WHERE ID = ?",(Name, Id))
        c.execute("UPDATE Students SET Roll = ? WHERE ID = ?",(roll, Id))
    else:
    	params = (Id, Name, roll)                                               # insering a new student data
    	c.execute("INSERT INTO Students(ID, Name, Roll) VALUES(?, ?, ?)", params)
